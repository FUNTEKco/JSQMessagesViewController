...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *collectionViewBottomDistanceConstraint;
...
- (IBAction)scrollToBottomButtonPressed:(id)sender {
    [self scrollToBottomAnimated: YES];
}

- (void)setupScrollToBottomButton {
    self.scrollToBottomButton.backgroundColor = [[UIColor grayColor] colorWithAlphaComponent:0.5];
    self.scrollToBottomButton.clipsToBounds = YES;
    self.scrollToBottomButton.layer.cornerRadius = 6.0;
    self.scrollToBottomButton.layer.borderColor = [[[UIColor darkGrayColor] colorWithAlphaComponent:0.5] CGColor];
    self.scrollToBottomButton.layer.borderWidth = 1.0;
    self.scrollToBottomButton.hidden = YES;
    [self.scrollToBottomButton setImage:[UIImage imageNamed:@"icon_arrow_down3"] forState:UIControlStateNormal];
}
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    BOOL hasShowScrollToBottomButton = ([self currentContentOffsetDiff] > 1000.0);
    if (hasShowScrollToBottomButton && ![self isOnScrollViewBottom:scrollView]) {
        self.scrollToBottomButton.hidden = NO;
    } else {
        self.scrollToBottomButton.hidden = YES;
    }
    self.isOnCollectionviewBottom = ([self currentContentOffsetDiff] < 650.0);
}
- (CGFloat)currentContentOffsetDiff {
    CGFloat currentContentOffsetY = [_collectionView contentOffset].y;
    CGFloat currentContentSizeY = [_collectionView contentSize].height - self.topContentAdditionalInset - self.bottomContentAdditionalInset;
    CGFloat currentContentOffDiff = (currentContentSizeY - currentContentOffsetY);
    return currentContentOffDiff;
}
- (BOOL)isOnScrollViewBottom:(UIScrollView *)scrollView {
    float scrollViewHeight = scrollView.frame.size.height;
    float scrollContentSizeHeight = scrollView.contentSize.height;
    float scrollOffset = scrollView.contentOffset.y;
    BOOL isOnBottom = (scrollOffset + scrollViewHeight >= scrollContentSizeHeight);
    return isOnBottom;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (@available(iOS 11.0, *)) {
        ...
        NSLog(@"kimura check bottomPadding = %f", bottomPadding); //kimuranow
        _collectionViewBottomDistanceConstraint.constant = bottomPadding;
...
{
    CGFloat toolbarBottomDistance = constant;

    if (@available(iOS 11.0, *)) {
        UIWindow *window = UIApplication.sharedApplication.keyWindow;
        CGFloat bottomPadding = window.safeAreaInsets.bottom;
        toolbarBottomDistance += bottomPadding;
    }

    self.toolbarBottomLayoutGuide.constant = toolbarBottomDistance;
