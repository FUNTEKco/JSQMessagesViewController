...
- (void)jsq_configureFlowLayout
{
    ...
    _messageBubbleTextViewTextContainerInsets = UIEdgeInsetsMake(12.0f, 14.0f, 12.0f, 14.0f);
...
- (CGSize)messageBubbleSizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGSize resultSize = [self.bubbleSizeCalculator messageBubbleSizeForMessageData:messageItem
                                                                       atIndexPath:indexPath
                                                                        withLayout:self];
    //    if message.message.firstUrlParsed != nil {
    CGFloat linkPreviewFixedWidth = [UIScreen mainScreen].bounds.size.width * (300.0 / 414.0);
    CGSize resultSizeWithLinkPreviewFixedWidth = CGSizeMake(300.0, resultSize.height);

    NSString *messageText = [messageItem text];
    ...
    for (NSTextCheckingResult *match in matches) {
        if ([match resultType] == NSTextCheckingTypeLink) {
            return resultSizeWithLinkPreviewFixedWidth; //kimuranow
        }
    }
...
- (void)jsq_configureMessageCellLayoutAttributes:(JSQMessagesCollectionViewLayoutAttributes *)layoutAttributes
{
    ...
    layoutAttributes.messageBubbleContainerViewWidth = messageBubbleSize.width; //kimuranow
//    layoutAttributes.messageBubbleContainerViewWidth = 300.0;
